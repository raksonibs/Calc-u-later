...
public class GraphModel extends ApplicationFrame
{
	...
	double precision = 0.01; // This will affect how smooth the graph is, but
								// also the performance
	double tolerance = 2.5; // How much a point can differ from a previous point
							// before being set to null
	...
	public ChartPanel getChartPanel()
	{
		...
		for (double i = xLeft; i < xRight; i = i + precision)
		{
			...
			e = new ExpressionBuilder(expressionString).variables("X", "y")
					.build().setVariable("X", i);
			double result = e.evaluate();

			if (expression.contains("X")) // If we're dealing with a variable we
											// have to account for asymptotes
			{
				/*
				 * We check if the differences between two values starts to get
				 * too large If its getting really big really quickly, its
				 * likely approaching an asymptote This can cause issues with
				 * things like factorial, in which case the tolerance needs to
				 * be adjusted to compensate.
				 */
				current = result;
				difference = Math.abs(current - previous); // Check the absolute
															// value of the
															// difference
															// between current
															// result and
				...
				{
					System.out.println("Out of specified bounds"); // If out of
																	// bounds,
																	// don't
																	// connect
																	// the
			...
			// Other if theres no variables, were just drawing a straight line
			{
				series.add(i, result);
			}
		...
		JFreeChart chart = ChartFactory.createXYLineChart(expression, "X", // X-axis
																			// Name
				"Y", // Y-axis Name
				data, // Dataset
	...
	public void reset()
	{
		precision = 0.01; // This will affect how smooth the graph is, but also
							// the performance
		tolerance = 2.5; // How much a point can differ from a previous point
							// before being set to null
